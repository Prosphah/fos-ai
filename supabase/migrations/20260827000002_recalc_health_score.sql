-- Recalculate financial_health_score inside sync_financial_profile
-- so the score stays current after every transaction.
...
DECLARE
  ...
  -- debt-ratio inputs (static from onboarding)
...
BEGIN
  ...
  -- Get existing profile (static fields + risk_score)
  SELECT
    id,
  ...
  IF v_profile_id IS NOT NULL THEN
    ...
    -- Financial health score (same formula as calculateFinancialHealthScore in TS)
    v_fhs := ROUND(LEAST(
